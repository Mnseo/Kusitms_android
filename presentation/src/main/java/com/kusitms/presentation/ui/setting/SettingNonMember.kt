...
fun SettingNonMemberColumn1(navController: NavHostController) {
    ...
    val nonMemberSetting = getNonMemberSetting(navController)
        LazyColumn(
            modifier = Modifier
                .fillMaxSize()
            ...
            items(nonMemberSetting) { item->
                SettingTitle(
                    title = item.title,
                    onClick = {
                        if(item.url.isNotEmpty()) { openUriSetting(item.url, uriHandler) } else item.onClick()
                    }
                )
            }
        }
